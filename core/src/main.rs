...
async fn run_server(settings: Settings, dir: &std::path::Path) -> Result<(), CoreError> {
    // TODO(fase futura): geometría real vía XRandR en vez de un tamaño fijo
    // — ver limitación documentada en `server::peer_desktop`.
    let geometry = ionconnect_screen::MonitorGeometry::new(0, 0, 1920, 1080);
